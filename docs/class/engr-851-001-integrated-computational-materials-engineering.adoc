...
#Maria-Lee's research needs to find interfacial energy between grain boundaries.
====
...
.Baumann's ISV model can capture the changing boundary conditions of the same specimen when strained at stem:[800\degreeC] up to some deformation, stopped, and further strained under stem:[20\degreeC]. Conventional models cannot fit this experimental data, but ISV can.
...
Team iron (bcc) will be comprised of Joby, Noel,#Daniel-Kenney, and#Maria-Lee.
====
