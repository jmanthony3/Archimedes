...
|:--:|
| Recall that the stress field is loosely related to $5\burgers$. If you can squish smaller atoms into the solute, then the smaller atoms want to rise to the top of the dislocation which minimizes the local stress to straighten the dislocation. |
...
This only occurs for loading that results in differential effect: e.g. not for $\millerPlane{1}{1}{1}$ loading.
...
Using Al-Cu as the archetype, the Cu will first form in clusters along Al $\millerPlaneFamily{1}{0}{0}$ planes: aka [[Guinier-Preston-Zones]] (#GPZ).
