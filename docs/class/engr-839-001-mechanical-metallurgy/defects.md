...
If we consider material failure when a crystal fails, then we must shift the #nearest-neighbor of all atoms on a #slip-planes simultaneously.
...
|:--:|:--:|
| Often thought as extra "half-plane" of atoms: the **[Burger's Vector](burgers-vector.md)**. | Helical interruptions of the crystal lattice. |
...
If [Burger's Vector](burgers-vector.md), $\vec{b}$ is perpendicular to dislocation line, $l$, then it is an #edge-dislocations.
If [Burger's Vector](burgers-vector.md), $\vec{b}$ is parallel to dislocation line, $l$, then it is a #screw-dislocations.
...
#dislocation-motion along #slip-planes is called #glide.
#dislocation-motion perpendicular to #slip-planes is called #climb.
Transfer of dislocation from one #slip-planes to another is called #cross-slip.
...
|:--:|
| Dislocation motion increments by 1 [Burger's Vector](burgers-vector.md). |
...
Stress-strain relationship according to [[Hookes-Law]].
Displacement dictated by [Burger's Vector](burgers-vector.md).
...
Overall motion of one [Burger's Vector](burgers-vector.md), $\vec{b}$.
...
|:--:|
| [Burger's Vector](burgers-vector.md), $\vec{b}_{1}$ is mathematically equivalent to addition of two partial dislocations, $\vec{b}_{2}$ and $\vec{b}_{3}$. The scalar product should be zero in a single plane where $[111] \perp (111)$. |
...
|:--:|
| [Burger's Vector](burgers-vector.md) in same direction as #edge-dislocations and perpendicular as #screw-dislocations. The #edge-dislocations tries to move the #glide-plane out-of-plane to another #glide-plane. The #screw-dislocations allows the #glide-plane to move in-plane and #cross-slip. |
...
#dislocation-motion by 1 [Burger's Vector](burgers-vector.md), but that motion introduces a [[kink]] or [[jog]] in the lattice structure.
...
### Jogs and Kinks
[[jog]]
: Shifted segment of "tree" transfers to another #slip-planes.

[[kink]]
: Shifted segment of "tree" stays in same plane.

Created the same way, but the only difference is to which #slip-planes a dislocation moves to.

!!! example A [[kink]] dislocation forest at a #screw-dislocations moves all the trees the same, plane direction: [Burger's Vector](burgers-vector.md). When moving to another plane. Every piece of the tree has the same [Burger's Vector](burgers-vector.md).
...
|:--:|:--:|
| #edge-dislocations moves parallel to [Burger's Vector](burgers-vector.md). | [burgers-vector for #screw-dislocations in line of dislocation (#right-hand-rule) |



### Effects on #dislocation-motion
When two dislocations intersect, the mobility of dislocations afterward is important for [work-hardening](deformation-and-work-hardening.md).
...
|:--:|
| Direction, line vector of dislocation, and [Burger's Vector](burgers-vector.md). |
...
### Orowan Equation
Cumulative effect of dislocations can be modeled as a #unit-cube with parallel dislocations moving in one direction with all contributing a step size of [Burger's Vector](burgers-vector.md).
The #dislocation-density, [Burger's Vector](burgers-vector.md), and average distance travelled will all affect the total strain, $\tau$.
...
As the angle increases, dislocation interaction cause the energy to maximize around $30\degree$.
...
More likely to occur after extensive [work-hardening](deformation-and-work-hardening.md), low-temperature, high strain-rates: e.g. tin cry when bending a piece of tin.
...
|:--:|
| As #SFE decreases, the propensity toward #twinning increases. [work-hardening](deformation-and-work-hardening.md) rate drops in copper, but allows brass to stabilize its stress state. |
...
Twins will act as slip barrier, which increases the [work-hardening](deformation-and-work-hardening.md) rate of the material.
...
#grain-boundary must maintain contact under deformation; therefore, #slip must occur, which implies that [work-hardening](deformation-and-work-hardening.md) is proportional to #grain-boundary-area.
