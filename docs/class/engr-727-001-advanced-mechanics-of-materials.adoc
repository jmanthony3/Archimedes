...
.Usually we design for the _#yield-stress _ of a material because we like linear. Failure depends on your criterion.
...
These lead to *principal stresses* which are stem:[180\degree] apart (see xref:fig-mohrs_circle[]):
...
Strain-Hardening:: True stress increases continuously until sufficient plastic strain allows for the material to begin [necking](necking.md).
...
.Problem 5: At a point in a stressed body, the strain, related to the coordinate set stem:[xyz], are given by: stem:[\begin{bmatrix}200 & 300 & 200 \\\ 300 & -100 & 500 \\\ 200 & 500 & -400\end{bmatrix}\mu] Determine (a) the strain invariants; (b) the normal strain in the stem:[x'] direction, which is directed at angle stem:[\theta = 30\degree] from the stem:[x]-axis; (c) the principal strain stem:[\epsilon_{1}], stem:[\epsilon_{2}], and stem:[\epsilon_{3}]; and, (d) the maximum #shear-strain.
[stem, role=center]
++++
\begin{split}
